    ...
    <x-layout-component>
        ...
        <form
            method="POST"
            action="{{ route('tenants.update', ['tenant' => $tenant]) }}"
        >
            @csrf
            @method('PUT')
            <div class="space-y-12 sm:space-y-16">
                <div>
                    <h2 class="text-base/7 font-semibold text-gray-900 dark:text-white"> {{ $tenant->name }}</h2>
                    <p class="mt-1 max-w-2xl text-sm/6 text-gray-600 dark:text-gray-400">{{ __('Edit tenant details.') }}</p>
                    ...
                    <div class="mt-10 space-y-8 border-b border-gray-900/10 pb-12 sm:space-y-0 sm:divide-y sm:divide-gray-900/10 sm:border-t sm:border-t-gray-900/10 sm:pb-0 dark:border-white/10 dark:sm:divide-white/10 dark:sm:border-t-white/10">
                        ...
                        <x-forms.input-text
                            ...
                            name="birthday"
                            autocomplete="bday"
                            value="{{ old('birthday', $tenant->bithday) }}"
                        >
                            {{ __('Date of birth') }}
                        </x-forms.input-text>
                        ...
                        <x-forms.input-text
                            ...
                            name="account_number"
                            autocomplete="on"
                            value="{{ old('account_number', $tenant->account_number) }}"
