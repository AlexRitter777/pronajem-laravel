...
<x-layout-component>
    ...
    <form
        method="POST"
        action="{{ route('landlords.update', ['landlord' => $landlord]) }}"
    >
        @csrf
        @method('PUT')
        <div class="space-y-12 sm:space-y-16">
            <div>
                <h2 class="text-base/7 font-semibold text-gray-900 dark:text-white"> {{ $landlord->name }}</h2>
                ...
                <div class="mt-10 space-y-8 border-b border-gray-900/10 pb-12 sm:space-y-0 sm:divide-y sm:divide-gray-900/10 sm:border-t sm:border-t-gray-900/10 sm:pb-0 dark:border-white/10 dark:sm:divide-white/10 dark:sm:border-t-white/10">
                    ...
                    <x-forms.input-text
                        ...
                        name="birthday"
                        autocomplete="bday"
                        value="{{ old('birthday', $landlord->birthday) }}"
                    >
                        {{ __('Date of birth') }}
                    </x-forms.input-text>
    ...
    <x-modal.tw-modal>
        ...
        <form method="post" action="{{ route('landlords.destroy', $landlord) }}" >
            @csrf
            @method('delete')
            <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 id="dialog-title" class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Delete Landlord') }}</h3>
                    <div class="my-2">
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            {!! __('Are you sure you want to delete this landlord: ') .'<b>'. $landlord->name . '</b>?' !!}
                        </p>
                    </div>
                </div>
            </div>
