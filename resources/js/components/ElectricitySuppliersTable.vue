...
<table class="relative min-w-full divide-y divide-gray-300 dark:divide-white/15">
    ...
    <tr
        v-for="electricitySupplier in items"
        :key="electricitySupplier.id"
        class=" hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
    >
        <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-0 dark:text-white">
            <a
                :href="getItemUrl(electricitySupplier.id, 'show')"
            >
                {{ electricitySupplier.name }}
            </a>
        </td>
        <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">
            {{ electricitySupplier.description ?? "\u2014" }}
        </td>
        <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">
            {{ properties[electricitySupplier.id] }}
        </td>
        <td class=" flex justify-end py-4 pr-4 pl-3 text-sm whitespace-nowrap sm:pr-0">
            ...
            <a
                :href="getItemUrl(electricitySupplier.id, 'edit')"
